...
export function JobCard({ job, highlighted = false }: JobCardProps) {
  return (
    <div className="group h-34 w-79 rounded-md border border-[#E2E8F0] bg-[#FFFFFF] px-3 py-4 shadow-[0_1px_0_rgba(16,24,40,0.02)] transition-colors">
      <h3 className="text-foreground text-[14px] leading-5 font-medium tracking-normal lg:text-[18px]">
        {job.title}
      </h3>
      ...
      <button
        type="button"
        className="group-hover:text-foreground mt-8 inline-flex items-center justify-end gap-2 text-[14px] leading-4 text-[#36394A]"
      >
        <span>Apply</span>
        <MoveRight size={16} />
      </button>
